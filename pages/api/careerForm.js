...
handler.post(async (req, res) => {
  ...
  try {
    const {name, email, linkcv} = req.body
    const interested = req.body['interested[]']
    const emailTest = await transporter.sendMail({
      from: process.env.SMTP_USER,
      to: 'workinvs@gmail.com',
      subject: `Contact form submission from ${email}`,
      html: `<p style="font-size: 25px; color: black; text-align: center">You have a new <strong style="color: cornflowerblue">Career Form</strong> form submission</p>
             ...
             <p style="font-size: 20px; color: black"><strong style="color: interested in">Link to your CV:  </strong>${linkcv != "" ? linkcv : ''}  </p>`,
      attachments: [
        {
